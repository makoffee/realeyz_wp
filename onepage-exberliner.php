<?php
/*
 * Template Name: Exberliner Landing Page
 */
get_header();
?>
<?php

	// checking menu exist in location "exberliner"
	if(  ( $locations = get_nav_menu_locations() ) && $locations['exberliner'] )
	{
		$menu 			= wp_get_nav_menu_object( $locations['exberliner'] );
	...
	if (have_posts()) {
		...
		while ( $allPosts->have_posts() ) { $allPosts->the_post();
			...
			if(( $separator != 1 ))
			{
				...
				?>
					<section id="<?php echo $post->post_name; ?>" class="<?php echo $pad_class; ?>" <?php if($background_color != ""): ?> style='background-color:<?php echo $background_color ?>;'<?php elseif($image !=""): ?> style='background-image:url(<?php echo $image ?>); background-size:cover; '<?php endif; ?>>
						<?php if( $no_title != 1 ){ ?>
